...
static void GC0339_Sensor_Init(void)
{
	...
	GC0339_write_cmos_sensor(0x14,0x23); // mirror:01/ updown:32 /mirror+updown:23
	GC0339_write_cmos_sensor(0x1a,0x21);
	GC0339_write_cmos_sensor(0x1b,0x08); // 88 20131018
	GC0339_write_cmos_sensor(0x1c,0x19); // 19 20131018
	GC0339_write_cmos_sensor(0x1d,0xea);
	
	GC0339_write_cmos_sensor(0x30,0xb7); // add 20130913 lanking
	GC0339_write_cmos_sensor(0x31,0x7f); // 20131018 lanking
	...
	GC0339_write_cmos_sensor(0x42,0x20);  //28  20130913 lanking
	GC0339_write_cmos_sensor(0x47,0x10);
	GC0339_write_cmos_sensor(0x50,0x40);// 40  
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	for (i=0; i<255; i++)
	{
		printk("%s 0x%00x\n", __func__,(int)GC0339_read_cmos_sensor(i));
	}
#if defined(MT6577)||defined(MT6589)
	spin_unlock(&gc0339_drv_lock);
#endif
...
void GC0339GetAFMaxNumFocusAreas(UINT32 *pFeatureReturnPara32)
{	
    *pFeatureReturnPara32 = 0;    
    SENSORDB(" GC0339GetAFMaxNumFocusAreas, *pFeatureReturnPara32 = %d\n",  *pFeatureReturnPara32);

}

void GC0339GetAFMaxNumMeteringAreas(UINT32 *pFeatureReturnPara32)
{	
    *pFeatureReturnPara32 = 0;    
    SENSORDB(" GC0339GetAFMaxNumMeteringAreas,*pFeatureReturnPara32 = %d\n",  *pFeatureReturnPara32);

}

void GC0339GetExifInfo(UINT32 exifAddr)
{
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_GET_AF_MAX_NUM_FOCUS_AREAS:
			GC0339GetAFMaxNumFocusAreas(pFeatureReturnPara32);
			*pFeatureParaLen=4;
		break;
		case SENSOR_FEATURE_GET_AE_MAX_NUM_METERING_AREAS:
			GC0339GetAFMaxNumMeteringAreas(pFeatureReturnPara32);
			*pFeatureParaLen=4;
		break;
		case SENSOR_FEATURE_GET_EXIF_INFO:
			SENSORDB("SENSOR_FEATURE_GET_EXIF_INFO\n");
			GC0339GetExifInfo(*pFeatureReturnPara32);
			break;
