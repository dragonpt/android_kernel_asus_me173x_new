...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	400 //100
	#define STK_LT_N_CT	150 //50
	#define STK_HT_N_CT	180 //70
		...
		case SENSOR_GET_DATA:
			...
			{				
				...
				else
				{
					...
					else if(obj->als < 50 && abs(obj->als - obj->als_last) >= 20) //add by lgw
					{
					obj->als_last = obj->als;
					sensor_data->values[0] = stk3x1x_get_als_value(obj, obj->als);
					}
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("zhaoshaopeng %s: driver version: %s\n", __FUNCTION__, DRIVER_VERSION);
...
#endif
    APS_LOG("### leanda end stk3100 probe ok \r\n");
	stk3310_init_flag = 0;
